...
angular.module('simulations', [
  ...
  'simulation',
  'SimulationsData',
  function ($scope, simulation, SimulationsData) {
    $scope.scopes = [
      {
        name: 'Dzienny',
        id: 1
    ...
    $scope.simulation = { scope: $scope.scopes[1] };
    ...
    $scope.$watch('simulation.scope', function () {
      if ($scope.simulation.scope)
        SimulationsData($scope.simulation.scope.id).then(function (simulation) {
    ...
    function refreshData(data) {
      ...
      $scope.options.series.push({
        name: 'Historycznie dodany przych\xf3d',
        data: []
      });
      expectedOutcome();
      expectedIncome();
